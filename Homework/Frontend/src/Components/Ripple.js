...
import day1 from './XRP/Prediction_1days.png';
import day3 from './XRP/Prediction_3days.png';
import day5 from './XRP/Prediction_5days.png';
...
export const RipplePage = () => {
  ...
  useEffect(() => {
    fetch('results/XRP/Prediction_1days.txt')
      .then((response) => {
        if (response.ok) {
  ...
  useEffect(() => {
    fetch('results/XRP/Prediction_3days.txt')
      .then((response) => {
        if (response.ok) {
          return response.text();
        } else {
  ...
  useEffect(() => {
    fetch('results/XRP/Prediction_5days.txt')
      .then((response) => {
        if (response.ok) {
          return response.text();
        } else {
  ...
  return (
    <>
      <header>
        XRP <Button onClick={clickToHomePage}>Back to Homepage</Button>
      </header>
      <p></p>
