...
import day1 from './ADA/Prediction_1days.png';
import day3 from './ADA/Prediction_3days.png';
import day5 from './ADA/Prediction_5days.png';

export const CardanoPage = () => {
  ...
  useEffect(() => {
    fetch('results/ADA/Prediction_1days.txt')
      .then((response) => {
        if (response.ok) {
  ...
  useEffect(() => {
    fetch('results/ADA/Prediction_3days.txt')
      .then((response) => {
        if (response.ok) {
          return response.text();
        } else {
  ...
  useEffect(() => {
    fetch('results/ADA/Prediction_5days.txt')
      .then((response) => {
        if (response.ok) {
          return response.text();
        } else {
  ...
  return (
    <>
      <header>
        ADA <Button onClick={clickToHomePage}>Back to Homepage</Button>
      </header>
      <p></p>
