...
import day1 from './BTC/Prediction_1days.png';
import day3 from './BTC/Prediction_3days.png';
import day5 from './BTC/Prediction_5days.png';
...
export const BitcoinPage = () => {
  ...
  useEffect(() => {
    fetch('results/BTC/Prediction_1days.txt')
      .then((response) => {
        if (response.ok) {
  ...
  useEffect(() => {
    fetch('results/BTC/Prediction_3days.txt')
      .then((response) => {
        if (response.ok) {
          return response.text();
        } else {
  ...
  useEffect(() => {
    fetch('results/BTC/Prediction_5days.txt')
      .then((response) => {
        if (response.ok) {
          return response.text();
        } else {
  ...
  return (
    <>
      <header>
        BTC <Button onClick={clickToHomePage}>Back to Homepage</Button>
      </header>
      <p></p>
